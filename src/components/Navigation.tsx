            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="ghost" size="sm" className="relative">
                  ...
                  {state.totalItems > 0 && <Badge variant="default" className="absolute -top-2 -right-2 h-5 w-5 flex items-center justify-center p-0 text-xs min-w-[20px]">
                      {state.totalItems}
                    </Badge>}
                </Button>
              </SheetTrigger>
              <SheetContent className="w-full sm:max-w-md">
                ...
                <div className="flex flex-col h-full">
                  {state.items.length === 0 ? <div className="flex-1 flex items-center justify-center">
                      ...
                      <div className="flex-1 overflow-y-auto space-y-4 py-4">
                        {state.items.map(item => <div key={item.id} className="flex items-start space-x-3 p-3 rounded-lg border">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center space-x-2 mt-2">
                                <Button variant="outline" size="sm" className="h-8 w-8 p-0" onClick={() => updateQuantity(item.id, Math.max(0, item.quantity - 1))}>
                                ...
                                <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-destructive hover:text-destructive ml-2" onClick={() => removeFromCart(item.id)}>
                      ...
                      <SheetFooter className="flex-col space-y-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full">
                          ...
                          <Button variant="outline" className="w-full sm:w-auto sm:min-w-[120px] h-11" onClick={clearCart}>
                            Clear Cart
                          </Button>
                        </div>
                      </SheetFooter>
